...
記帳する仕訳の各明細に、freee の取引ID（`imported_tx_id` タグ）を残す。再実行時は
**まずこのIDで突き合わせる**ので、同じ内容の別取引（同じ日に同額の交通費が2件など）
と取り違えない。

IDが無い仕訳——このタグを付ける前に記帳した分——は、取引日・摘要・明細で作った
指紋を**件数で**突き合わせる。freee に3件・帳簿に2件なら1件だけ記帳する。
ID だけで判定すると、古い月を流し直したときに全件が二重計上になる。

そのため、月の途中で何度流しても新しい取引だけが入る。

### 手で入れた仕訳は食い違いとして出る

kaikei 側だけにある仕訳（決算整理、受取利息の事業主借への振替など）は freee の
サマリーに無いので、突き合わせで食い違いとして出る。**道具は事実を報告している**
ので、内容を見て判断すること。
