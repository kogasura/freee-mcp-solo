...
個人事業主の日常経理に特化した [freee会計](https://www.freee.co.jp/) 用 MCP (Model Context Protocol) サーバー。
...
```bash
npm install -g freee-accounting-mcp
```

または、ソースからビルド:

```bash
git clone https://github.com/kogasura/freee-accounting-mcp.git
cd freee-accounting-mcp
npm install
npm run build
```
...
```bash
mkdir -p ~/.config/freee-accounting-mcp
cp config.example.yaml ~/.config/freee-accounting-mcp/config.yaml
```
...
```json
{
  "mcpServers": {
    "freee-accounting": {
      "type": "stdio",
      "command": "node",
      "args": ["/path/to/freee-accounting-mcp/dist/index.js"]
    }
  }
}
```
